...
function load(){
  ...
  ['cal','deficit','weight','height','age'].forEach(k=>{const el=document.getElementById('g-'+k);if(el&&GOAL[k])el.value=GOAL[k];});
...
/* iOS Shortcut URL handler
   URL format: ?steps=8500&walkdist=6.2&swimmin=35&swimdist=1400&actcal=480&date=2026-04-07 */
function handleURLParams(){
  const p=new URLSearchParams(window.location.search);
  if(!p.has('steps')&&!p.has('actcal')&&!p.has('swimmin'))return;
  ...
  setTimeout(()=>showToast('✅ נתוני Apple Watch נשמרו ל-'+fmtDate(date)),500);
}

function showToast(msg){
  let t=document.getElementById('toast');
  if(!t){t=document.createElement('div');t.id='toast';
    t.style.cssText='position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#1a1916;color:#fff;padding:10px 18px;border-radius:99px;font-size:13px;z-index:9999;transition:opacity .4s;white-space:nowrap;pointer-events:none';
...
const FOOD_DB=[
  ...
  {k:['עוגה'],cal:300},{k:['קרקר'],cal:130},{k:['חטיף','צ\'יפס'],cal:280},
...
function renderLog(){
  ...
  const wDay=dd(curD);
  const wi=document.getElementById('weight-input');if(wi)wi.value=wDay.weight||'';
...
function renderCal(){
  ...
  if(wt){
    ...
    if(wpts.length>=2){
      ...
      wt.innerHTML='<div class="ws-title">מגמת משקל</div><div class="empty-note">הזן משקל לפחות יומיים כדי לראות מגמה</div>';
    }
  }
...
function renderAdv(){
  ...
  if(avgOut<300)mHtml+=`<div class="adv-card warn-card"><h3>הגדל פעילות <span class="adv-tag a">חשוב</span></h3><p>שריפה ממוצעת נמוכה (${r(avgOut)} קק"ל). הוסף 30 דק' הליכה מהירה ביום.</p></div>`;
  if(avgIn>GOAL.cal+150)mHtml+=`<div class="adv-card warn-card"><h3>הפחת צריכה <span class="adv-tag r">דחוף</span></h3><p>הצריכה עולה על יעדך ב-${r(avgIn-GOAL.cal)} קק"ל.</p></div>`;
  mHtml+=`<div class="adv-card"><h3>📅 תוכנית ל-3 ימים</h3>
    <div class="plan-day"><h4>יום 1</h4><div class="plan-row">בוקר: ביצים + ירקות — 280 קק"ל</div><div class="plan-row">צהריים: עוף + קינואה + סלט — 480 קק"ל</div><div class="plan-row">ערב: דג + ירקות — 350 קק"ל</div><div class="plan-row">פעילות: 35 דק' הליכה</div></div>
    <div class="plan-day"><h4>יום 2</h4><div class="plan-row">בוקר: יוגורט יווני + פירות — 260 קק"ל</div><div class="plan-row">צהריים: סלט טונה + לחם מלא — 400 קק"ל</div><div class="plan-row">ערב: מרק עוף — 280 קק"ל</div><div class="plan-row">פעילות: שחייה 30 דק'</div></div>
    <div class="plan-day"><h4>יום 3</h4><div class="plan-row">בוקר: שיבולת שועל + בננה — 300 קק"ל</div><div class="plan-row">צהריים: עוף + אורז מלא — 500 קק"ל</div><div class="plan-row">ערב: חביתה + ירקות — 260 קק"ל</div><div class="plan-row">פעילות: כושר 45 דק'</div></div>
  </div>`;
  ac.innerHTML=mHtml;

  if(as)as.innerHTML=`
    <div class="adv-card"><h3>💡 טיפים</h3><p>• 2–3 ליטר מים ביום<br>• אכול לאט<br>• 8,000+ צעדים = ~320 קק"ל<br>• שינה 7–8 שעות<br>• חלבון בכל ארוחה</p></div>
    <div class="adv-card"><h3>⌚ iOS Shortcut URL</h3>
      <p style="font-size:12px;margin-bottom:8px">הדבק ב-"Open URL" בקיצור <strong>צעדים יומי</strong>:</p>
      ...
      <p style="font-size:11px;color:var(--muted);margin-top:8px">כל פרמטר בסוגריים מוחלף בערך אמיתי מ-Apple Health</p>
    </div>`;
}
